...
def discover_modules():
    ...
    # Registries live in engine, load that first
    from easyvolcap import engine  # noqa: F401

    allow_prefixes = ('easyvolcap.models', 'easyvolcap.dataloaders', 'easyvolcap.runners')

    # Eagerly import all easyvolcap.* packages so registry decorators run
    for _, name, _ in pkgutil.walk_packages(easyvolcap.__path__, easyvolcap.__name__ + '.'):
        if not name.startswith(allow_prefixes):  # skip utility modules that don't touch registries
            continue
        try:
            importlib.import_module(name)
        except Exception as e:
            log(yellow_slim(f'Failed to import {blue(name)}, {red(type(e).__name__)}: {red_slim(e)}'))
