...
class VolumetricVideoDataset(Dataset):
        ...
        if self.use_depths:  # TODO: implement HDR loading
            self.dps_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.dps, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.center_crop_size,
                                             ...
                                             decode_flag=cv2.IMREAD_UNCHANGED, dist_opt_K=self.dist_opt_K, backend=self.backend, encode_ext='.exr', num_workers=self.dataloading_workers)  # will for a grayscale read from bytes
    ...
    def load_cameras(self):
        ...
        # Cache the init view camera parameters for viewer initialization before view filtering
        self.Kv = self.Ks[self.init_viewer_index, 0].clone()
        self.Hv = self.Hs[self.init_viewer_index, 0].clone()
        self.Wv = self.Ws[self.init_viewer_index, 0].clone()
    ...
    def get_image_from_disk(self, view_index: int, latent_index: int, output: dotdict = None):
        ...
        if self.use_masks:
            ...
            mk, _, _, _ = load_resize_undist_im_bytes(mk, K.numpy(), D.numpy(), ratio, self.center_crop_size, decode_flag=cv2.IMREAD_GRAYSCALE, dist_opt_K=self.dist_opt_K, backend=self.backend, encode=False)
            mk = mk[..., None]

        # Maybe load background images here
        if self.use_bkgds:
            bg = self.bgs[view_index]
            bg, _, _, _ = load_resize_undist_im_bytes(bg, K.numpy(), D.numpy(), ratio, self.center_crop_size, dist_opt_K=self.dist_opt_K, backend=self.backend, encode=False)
        ...
        im, K, H, W = load_resize_undist_im_bytes(im, K.numpy(), D.numpy(), ratio, self.center_crop_size, dist_opt_K=self.dist_opt_K, backend=self.backend, encode=False)
